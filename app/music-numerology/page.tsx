...
function parabolicInterp(mags: Float32Array | Uint8Array, i: number) {
  const L = mags.length, i0 = Math.max(0, Math.min(L - 1, i));
  const im1 = Math.max(0, i0 - 1), ip1 = Math.min(L - 1, i0 + 1);
  const ym1 = (mags as any)[im1], y0 = (mags as any)[i0], yp1 = (mags as any)[ip1];
...
type LiveSample = { t: number; freq: number; midi: number | null; note: string; digit: number | null; cents?: number; };
...
export default function MusicNumerologyPage() {
  ...
  const [wHigh, setWHigh] = useState(0.20);
  ...
  const [liveLow,  setLiveLow]  = useState<{freq:number; midi:number|null; note:string; digit:number|null; cents?:number}|null>(null);
  const [liveMid,  setLiveMid]  = useState<{freq:number; midi:number|null; note:string; digit:number|null; cents?:number}|null>(null);
  const [liveHigh, setLiveHigh] = useState<{freq:number; midi:number|null; note:string; digit:number|null; cents?:number}|null>(null);
  ...
  const gloss: Record<number, string> = {
    1:"Will / ignition / first spark", 2:"Relating / listening / harmony of two", 3:"Expression / creativity / divine child",
    4:"Structure / order / grounded rhythm", 5:"Change / freedom / motion", 6:"Care / devotion / restoration",
    7:"Depth / insight / the seeker", 8:"Power / balance / stewardship", 9:"Compassion / completion / blessing",
  };
  const sumToLines = (counts: Record<number, number>) => {
    const total = Object.values(counts).reduce((a, b) => a + b, 0);
    if (!total) return { lines: ["No data yet."], lead: null as number | null };
    const top3 = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k,v])=>({digit:Number(k), pct:(v/total)*100}));
    return { lines: top3.map(d=>`• ${d.digit} ≈ ${d.pct.toFixed(0)}% — ${gloss[d.digit]}`), lead: top3[0]?.digit ?? null };
  };
  const makeReading = (lead: number | null, label: "inner" | "root" = "inner") => {
    const inner = label === "inner";
    if (inner) {
      return lead === 7 ? "Inner lines think deeply — clarity over noise."
        : lead === 3 ? "Inner lines want to speak — creation over silence."
        : lead === 9 ? "Inner lines heal — compassion over conflict."
        : lead === 4 ? "Inner lines build — order over chaos."
        : lead === 6 ? "Inner lines care — warmth over fracture."
        : lead === 5 ? "Inner lines move — change over stagnation."
        : lead === 8 ? "Inner lines center — integrity over excess."
        : lead === 2 ? "Inner lines listen — attunement over resistance."
        : lead === 1 ? "Inner lines ignite — beginnings over delay."
        : "Inner lines balance — aether weaving all elements.";
    } else {
      return lead === 7 ? "The root is reflective — depth underfoot."
        : lead === 3 ? "The root speaks — rhythmic expression."
        : lead === 9 ? "The root blesses — compassionate pulse."
        : lead === 4 ? "The root builds — grounded order."
        : lead === 6 ? "The root nurtures — protective warmth."
        : lead === 5 ? "The root roams — motion and release."
        : lead === 8 ? "The root steadies — strong spine."
        : lead === 2 ? "The root attunes — relational groove."
        : lead === 1 ? "The root ignites — primal spark."
        : "The root balances — quiet aether below.";
    }
  };
  const lowSummary  = useMemo(() => { const { lines, lead } = sumToLines(lowCounts);  return ["Body (200–500 Hz):", ...lines, "", `Reading: ${makeReading(lead,"inner")}`].join("\n");}, [lowCounts]);
  const midSummary  = useMemo(() => { const { lines, lead } = sumToLines(midCounts);  return ["Voice (500–1500 Hz):", ...lines, "", `Reading: ${makeReading(lead,"inner")}`].join("\n");}, [midCounts]);
  const highSummary = useMemo(() => { const { lines, lead } = sumToLines(highCounts); return ["Air (1500–6000 Hz):", ...lines, "", `Reading: ${makeReading(lead,"inner")}`].join("\n");}, [highCounts]);
  const bassSummary = useMemo(() => { const { lines, lead } = sumToLines(bassCounts); return ["Bass (60–300 Hz):", ...lines, "", `Reading: ${makeReading(lead,"root")}`].join("\n");}, [bassCounts]);
  ...
  async function ensureAudioGraph() {
    ...
    if (!sourceRef.current) sourceRef.current = ctxRef.current.createMediaElementSource(audioRef.current!);

    const splitter = ctxRef.current.createChannelSplitter(2);
    const gL = ctxRef.current.createGain(); gL.gain.value = 0.5;
    const gR = ctxRef.current.createGain(); gR.gain.value = 0.5;
    ...
    if (!analyserRef.current) {
      ...
      a.minDecibels = -100; a.maxDecibels = -10;
    ...
    if (!workletRef.current) {
      const n = new AudioWorkletNode(ctxRef.current, "bass-yin-processor", { numberOfInputs:1, numberOfOutputs:1, outputChannelCount:[1] });
      const mute = ctxRef.current.createGain(); mute.gain.value = 0;
      n.port.onmessage = (ev) => {
        if (freezeRef.current || !analyzingRef.current) return;
        const msg = ev.data;
        if (msg?.type === "bass") {
          const f0: number = msg.f0; const yinConf: number = msg.conf;
          ...
          setBassFreq(fSm); setBassMidi(midi); setBassNote(note); setBassDigit(digit); setBassCents(snapped.cents);
          const t = performance.now();
          const ts = Number(((t - (startTimeRef.current ?? t)) / 1000).toFixed(2));
          if (digit) {
            setBassSamples(prev => [...prev.slice(-1000), { t: ts, freq: fSm, midi, note, digit, cents: snapped.cents }]);
            setBassCounts(prev => ({ ...prev, [digit]: (prev[digit] ?? 0) + w }));
            setEmbodimentCounts(prev => ({ ...prev, [digit]: (prev[digit] ?? 0) + w * wBass }));
          }
        }
      };
      merger.connect(n); n.connect(mute).connect(ctxRef.current.destination);
      workletRef.current = n;
    }
  }

  function readHarmBand(ctx: AudioContext, analyser: AnalyserNode, A4: number, lowHz: number, highHz: number, emaRef: React.MutableRefObject<number|null>) {
    ...
    if (isPercussive) return { freq:0, midi:null, note:"-", digit:null, cents:0, conf:0 };
    ...
    let fLead = 0, leadW = -1, peakVote: Record<number, number> = {};
    if (peaks.length) {
    ...
    const noteSnap = midiToNoteName(midiSnap);  // ← fixed (removed stray "the:")
    ...
    for (let i = cutL; i <= cutH; i++) {
      ...
      const frac = pc - pc0;
      const left = (pc0 + 11) % 12, right = (pc0 + 1) % 12;
    ...
    let pcChroma = null as number | null, confChroma = 0;
    if (chromaNorm > 0) {
    ...
    if (timeBufRef.current) {
      ...
      for (let lag = minLag; lag <= maxLag; lag++) {
        ...
        if (sum > best) { next = best; best = sum; bestLag = lag; }
    ...
    if (agree) { wSnap *= 1.2; wChroma *= 1.2; }
    const wBoost = 1 + 0.5 * confAcf;
    wSnap *= wBoost; wChroma *= wBoost;

    let digit: number | null = null, conf = 0;
    if (digitSnap == null && digitChroma == null) {
      ...
      digit = digitSnap; conf = clamp(wSnap, 0, 1);
    } else {
      digit = digitChroma!; conf = clamp(wChroma, 0, 1);
    }

    return { freq:fSm, midi: midiSnap, note: noteSnap, digit, cents: snapped.cents, conf };
  }
  ...
  function tick() {
    ...
    setLiveLow(rLow); setLiveMid(rMid); setLiveHigh(rHigh);

    const t = Number(((now - startTimeRef.current) / 1000).toFixed(2));
    if (rLow.digit) {
      setLowSamples(prev => [...prev.slice(-800), { t, freq:rLow.freq, midi:rLow.midi, note:rLow.note, digit:rLow.digit, cents:rLow.cents }]);
      setLowCounts(prev => ({ ...prev, [rLow.digit!]: (prev[rLow.digit!] ?? 0) + (rLow as any).conf }));
      setEmbodimentCounts(prev => ({ ...prev, [rLow.digit!]: (prev[rLow.digit!] ?? 0) + (rLow as any).conf * wLow }));
    }
    if (rMid.digit) {
      setMidSamples(prev => [...prev.slice(-800), { t, freq:rMid.freq, midi:rMid.midi, note:rMid.note, digit:rMid.digit, cents:rMid.cents }]);
      setMidCounts(prev => ({ ...prev, [rMid.digit!]: (prev[rMid.digit!] ?? 0) + (rMid as any).conf }));
      setEmbodimentCounts(prev => ({ ...prev, [rMid.digit!]: (prev[rMid.digit!] ?? 0) + (rMid as any).conf * wMid }));
    }
    if (rHigh.digit) {
      setHighSamples(prev => [...prev.slice(-800), { t, freq:rHigh.freq, midi:rHigh.midi, note:rHigh.note, digit:rHigh.digit, cents:rHigh.cents }]);
      setHighCounts(prev => ({ ...prev, [rHigh.digit!]: (prev[rHigh.digit!] ?? 0) + (rHigh as any).conf }));
      setEmbodimentCounts(prev => ({ ...prev, [rHigh.digit!]: (prev[rHigh.digit!] ?? 0) + (rHigh as any).conf * wHigh }));
    }
  ...
  async function startAnalysis() {
    try {
      ...
      if (!audioRef.current) { setError("Audio element not ready."); return; }
      if (ctxRef.current?.state === "suspended") await ctxRef.current.resume();
      startTimeRef.current = null;

      emaLowRef.current = null; emaMidRef.current = null; emaHighRef.current = null;
      bassFreqEmaRef.current = null;

      setLowSamples([]); setMidSamples([]); setHighSamples([]); setBassSamples([]);
      setLowCounts({...zeroCounts}); setMidCounts({...zeroCounts}); setHighCounts({...zeroCounts});
      setBassCounts({...zeroCounts}); setEmbodimentCounts({...zeroCounts});
  ...
  function onFile(e: React.ChangeEvent<HTMLInputElement>){
    ...
    const url = URL.createObjectURL(f); fileUrlRef.current=url;
    ...
    setLowCounts({...zeroCounts}); setMidCounts({...zeroCounts}); setHighCounts({...zeroCounts});
    setBassCounts({...zeroCounts}); setEmbodimentCounts({...zeroCounts});
    emaLowRef.current=null; emaMidRef.current=null; emaHighRef.current=null; bassFreqEmaRef.current=null;
      ...
      <main className="px-4 py-8 max-w-6xl mx-auto">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-neutral-800 p-4 bg-neutral-900/60">
            ...
            <input type="file" accept="audio/*" onChange={onFile} className="mt-2 block w-full text-sm" />
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <button onClick={togglePlay} className="px-3 py-2 rounded-lg border border-neutral-700 hover:bg-neutral-800">
                {isPlaying ? "Pause" : "Play"}
              </button>
              {!isAnalyzing ? (
                <button onClick={startAnalysis} className="px-3 py-2 rounded-lg border border-yellow-600 text-yellow-400 hover:bg-yellow-500/10">
                  Start Analysis
                </button>
              ) : (
                <button onClick={stopAnalysis} className="px-3 py-2 rounded-lg border border-pink-600 text-pink-300 hover:bg-pink-500/10">
                  Stop Analysis
                </button>
              )}

              <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
                <input type="checkbox" checked={rotateToTonic} onChange={(e)=>setRotateToTonic(e.target.checked)} />
              ...
              <label className="inline-flex items-center gap-2 text-xs text-neutral-300 ml-3">
                A4:
                <input type="range" min={415} max={466} step={1} value={a4} onChange={(e)=>setA4(Number(e.target.value))}/>
            ...
            <div className="mt-3 grid gap-2">
              ...
              <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
                Low (Body): <input type="range" min={0} max={1} step={0.05} value={wLow} onChange={(e)=>setWLow(Number(e.target.value))}/>
                <span className="font-mono">{Math.round(wLow*100)}%</span>
              </label>
              <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
                Mid (Voice): <input type="range" min={0} max={1} step={0.05} value={wMid} onChange={(e)=>setWMid(Number(e.target.value))}/>
                <span className="font-mono">{Math.round(wMid*100)}%</span>
              </label>
              <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
                High (Air): <input type="range" min={0} max={1} step={0.05} value={wHigh} onChange={(e)=>setWHigh(Number(e.target.value))}/>
                <span className="font-mono">{Math.round(wHigh*100)}%</span>
              </label>
              <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
                Bass share into Embodiment: <input type="range" min={0} max={0.6} step={0.05} value={wBass} onChange={(e)=>setWBass(Number(e.target.value))}/>
                <span className="font-mono">{Math.round(wBass*100)}%</span>
              </label>
            </div>

            <div className="mt-2 text-xs text-neutral-400">
              Key: <b>{NOTE_NAMES[keyInfo.tonicPC]} {keyInfo.mode==="maj"?"(maj)":"(min)"}</b> · Conf: {Math.round(keyInfo.confidence)}%
            </div>
          ...
          <div className="rounded-xl border border-neutral-800 p-4 bg-neutral-900/60">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                ...
                <div className="font-mono mt-1">{liveLow?.freq ? `${liveLow.freq.toFixed(1)} Hz` : "-"}</div>
              </div>
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">Low Note</div>
                <div className="font-mono mt-1">{liveLow?.midi ?? "-"} / {liveLow?.note ?? "-"}{typeof liveLow?.cents === "number" ? ` (${fmtCents(liveLow.cents!)})` : ""}</div>
              </div>

              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">Mid Harmonic (Voice) Freq</div>
                <div className="font-mono mt-1">{liveMid?.freq ? `${liveMid.freq.toFixed(1)} Hz` : "-"}</div>
              </div>
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">Mid Note</div>
                <div className="font-mono mt-1">{liveMid?.midi ?? "-"} / {liveMid?.note ?? "-"}{typeof liveMid?.cents === "number" ? ` (${fmtCents(liveMid.cents!)})` : ""}</div>
              </div>

              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">High Harmonic (Air) Freq</div>
                <div className="font-mono mt-1">{liveHigh?.freq ? `${liveHigh.freq.toFixed(1)} Hz` : "-"}</div>
              </div>
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">High Note</div>
                <div className="font-mono mt-1">{liveHigh?.midi ?? "-"} / {liveHigh?.note ?? "-"}{typeof liveHigh?.cents === "number" ? ` (${fmtCents(liveHigh.cents!)})` : ""}</div>
              </div>

              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">Bass Freq</div>
                <div className="font-mono mt-1">{bassFreq ? `${bassFreq.toFixed(1)} Hz` : "-"}</div>
              </div>
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950">
                <div className="opacity-60 text-xs">Bass Note</div>
                <div className="font-mono mt-1">
                  {bassMidi ?? "-"} / {bassNote}{typeof bassCents === "number" ? ` (${fmtCents(bassCents)})` : ""}
                </div>
              </div>
              ...
              <div className="rounded-lg border border-neutral-800 p-3 bg-neutral-950 col-span-2">
                ...
                <div className="font-mono mt-1">{mainHarmonic.label} {mainHarmonic.label !== "-" ? `· ${mainHarmonic.confidence.toFixed(0)}%` : ""}</div>
              </div>
            </div>
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={()=>setExpandHarm(s=>!s)} className="text-xs px-2 py-1 rounded border border-neutral-700 hover:bg-neutral-800 inline-flex items-center gap-1">
                  {expandHarm ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}{expandHarm?"Collapse":"Expand"}
                </button>
              </div>
              <div className="flex gap-1 flex-wrap">
                {recentLow.map((s,i)=> <span key={`l${i}`} className="px-2 py-1 rounded-md text-xs font-mono border border-neutral-800 bg-neutral-950" title={`LOW ${s.note} ${s.cents!=null?`(${fmtCents(s.cents)}) `:""}@ ${s.freq.toFixed(1)}Hz • t=${s.t.toFixed(2)}s`}>{s.digit}</span>)}
                {recentMid.map((s,i)=> <span key={`m${i}`} className="px-2 py-1 rounded-md text-xs font-mono border border-neutral-800 bg-neutral-950" title={`MID ${s.note} ${s.cents!=null?`(${fmtCents(s.cents)}) `:""}@ ${s.freq.toFixed(1)}Hz • t=${s.t.toFixed(2)}s`}>{s.digit}</span>)}
                {recentHigh.map((s,i)=> <span key={`h${i}`} className="px-2 py-1 rounded-md text-xs font-mono border border-neutral-800 bg-neutral-950" title={`HIGH ${s.note} ${s.cents!=null?`(${fmtCents(s.cents)}) `:""}@ ${s.freq.toFixed(1)}Hz • t=${s.t.toFixed(2)}s`}>{s.digit}</span>)}
                {recentLow.length+recentMid.length+recentHigh.length===0 && <span className="text-xs text-neutral-500">—</span>}
              </div>

              <div className="flex items-center justify-between mt-3">
                <div className="opacity-60 text-xs mb-1">Recent digits — Bass</div>
                <button onClick={()=>setExpandBass(s=>!s)} className="text-xs px-2 py-1 rounded border border-neutral-700 hover:bg-neutral-800 inline-flex items-center gap-1">
                  {expandBass ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}{expandBass?"Collapse":"Expand"}
                </button>
              </div>
              <div className="flex gap-1 flex-wrap">
                {recentB.map((s, i) => (
                  <span key={`b${i}`} className="px-2 py-1 rounded-md text-xs font-mono border border-neutral-800 bg-neutral-950" title={`${s.note} ${s.cents!=null?`(${fmtCents(s.cents)}) `:""}@ ${s.freq.toFixed(1)}Hz • t=${s.t.toFixed(2)}s`}>
                    {s.digit}
                  </span>
                ))}
        ...
        <div className="mt-6 text-xs text-neutral-500">
          ...
          Embodiment blends their votes with your sliders and can include a portion of Bass.
          Key detection uses all harmonic samples; “Main Note” aggregates across the last ~{MAIN_NOTE_WINDOW} frames.
        </div>
      </main>
    </Shell>
  );
...
function MsgCard({ title, text, wide }: { title: string; text: string; wide?: boolean }) {
  return (
    <div className={`rounded-xl border border-neutral-800 p-4 bg-neutral-900/60 ${wide ? "md:col-span-2" : ""}`}>
